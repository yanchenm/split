...
const Expense: React.FC<ExpenseProps> = ({ name, paidBy, participants, total, date }) => {
  ...
  return (
    <div className="py-5 grid grid-cols-12 text-start font-normal text-slate-200">
          ...
          participants.map((person: any) => {
            return (
              <span className="mr-auto text-start" key={person.name}> {/* FIX KEY */}
                <h3 className="text-slate-400">{person.name}</h3>
                <h3 className="text-lg font-medium text-slate-200">${(person.portion*total).toFixed(2)}</h3>
              </span>
            )
          })
        }
